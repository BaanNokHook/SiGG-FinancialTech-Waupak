...
GM.Utility.DDLAutoComplete = function(ddl, data, txtid_for_setvalue, isSelect, text) {
    ...
    $.ajax({
        ...
        success: function(res) {
            ...
            if (res.length) {
                if (isSelected) {
                    ta = $("<a  onclick='ClearDDL(\"" + idtext + "\",\"" + defultText + "\" )'></a>").val("")
                        .html(defultText);
                }
            } else {
            ...
            $.each(res,
                function(i, resdata) {
                    var li = $("<li class='searchterm'></li>");
                    var ta = $("<a data-toggle='tooltip'  title='" +
                        encodeHTML(resdata.Text) +
                        "' onclick='AddTextForDDL(\"" +
                        encodeHTML(resdata.Text) +
                        "\",\"" +
                        idtext +
                        "\",\"" +
                        encodeHTML(resdata.Value) +
                        "\",\"" +
                        txtid_for_setvalue +
                        "\",\"" +
                        encodeHTML(resdata.Value2) +
                        "\")'></a>").val(encodeHTML(resdata.Value)).html(encodeHTML(resdata.Text));
                    li.append(ta);
                    ul.append(li);
                });
        },
...
GM.Utility.DDLAutoCompleteSet4Value =
    function(ddl, data, txtid_for_setvalue2, txtid_for_setvalue3, txtid_for_setvalue4) {
        var action = $(ddl).data("action");
        var idtext = $(ddl).attr("id");
        var ul = $("#" + idtext).parent().parent().find("ul");
        data.t = new Date().getTime();
                ...
                $.each(res,
                    function(i, resdata) {
                        ...
                        var ta = $("<a data-toggle='tooltip' title='" +
                            ...
                            txtid_for_setvalue3 +
                            "\",\"" +
                            encodeHTML(resdata.Value3) +
                            "\",\"" +
                            txtid_for_setvalue4 +
                            "\",\"" +
                            encodeHTML(resdata.Value4) +
                            "\")'></a>").val(encodeHTML(resdata.Value)).html(encodeHTML(resdata.Text));
                        li.append(ta);
                        ul.append(li);
                    });
...
function AddTextForDDL4Value(stringtext,
    idtxt,
    value,
    txtid_for_setvalue2,
    value2,
