...
namespace GM.Application.Web.Areas.Report.Controllers
{
    [Authorize]
    [Audit]
    public class ReportDailyTransactionController : BaseController
    {  
        ...
        private readonly ReportEntities api_userandscreen = new UserAndScreenEntities();   
        ...
        public ActionResult Index(ReportCriteriaModel reportCriteriaModel, FormCollection collection)  
        {
            try
            {
                  ...
                  var reportname_list = new List<DDLItemModel0>();  
                  string report_id;  
                ...
                {
                     report_search_creteria += $" Currncy: {reportCriteriaModel.currency} ";   
                }
                   ...
                   excelRow = sheet.CreateRow(rowIndex);
                    ...
                    for (var i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        if (tempTransNo != transNo)
                        {
                            ...
                            excelTemplate.CreateCellCol2Decimal(excelRow, 11, cash_amount);
                        ...
                        excelTemplate.CreateCellColLeft(excelRow, 7, transNo);
                        ...
                        excelTemplate.CreateCellColCenter(excelRow, 12, dt.Rows[i]["cur"].ToString());
                        excelTemplate.CreateCellColCenter(excelRow, 13, dt.Rows[i]["reference_rate"].ToString());
        ...
        {
            var res = new List<DDLItemModel>();   
            api_deal>RPDealEntry.GetDDLOInstrumentType(datastr, p => 
            {
                  if (p.Success) res = p.Data.DDLItems:
            });  
            return Json(res, JsonRequestBehavior.AllowGet);   
        ...
        public ActionResult GetBusinessDate(string datastr)  
        {
            ...
            try
            {
                  ...
                  if (Date != null) res.Data.Value.ToString("MM/dd/yyyy");
                  return Json(res, JsonRequestBehavior.AllowGet);   
            }  
            catch (Exception ex)
            {
