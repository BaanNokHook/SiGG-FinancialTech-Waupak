...
namespace GM.Application.Web.Controllers
{
    [Authorize]
    [Audit]
    public class RPDealSettlementController : BaseController
    {
        RPTransEntity api_RPTrans = new RPTransEntity();
        Utility utility = new Utility();
        private static string Controller = "RPDealSettlementController";
        private static LogFile Log = new LogFile();

        [RoleScreen(RoleScreen.VIEW)]
        public ActionResult Index()
        {
            var RPDealSettlementView = new RPDealEntryViewModel();
            RPTransModel RPTrans = new RPTransModel();

            RPDealSettlementView.FormSearch = RPTrans;

            return View(RPDealSettlementView);
        }
        ...
        public ActionResult Search(DataTableAjaxPostModel model)
        {
            ...
            try
            {
                ...
                api_RPTrans.RPDealSettlement.GetRPDealSettlementList(RPTransModel, p =>
                {
                    result = p;
                });
        ...
        public ActionResult Approve(string trans_no)
        {
            ...
            try
            {
                ...
                api_RPTrans.RPDealSettlement.GetRPDealSettlementDetail(RPTransModel, p =>
                {
                    result = p;
                });
                ...
                if (RPTransModel.trans_state.ToUpper() == "BO-APPROVE" || RPTransModel.trans_state.ToUpper() == "BO-REJECTMATURITY")
                {
                    RPTransModel.btn_Approve = true;
                    RPTransModel.btn_UnApprove = true;
                    return View(RPTransModel);
                }
                else if (RPTransModel.trans_state.ToUpper() == "BO-SETTLEMENT")
                {
        ...
        public ActionResult Search_Colateral(DataTableAjaxPostModel model, string trans_no)
        {
            ...
            try
            {
                ...
                api_RPTrans.RPDealSettlement.GetRPDealSettlementColateralList(RPTransModel, p =>
                {
                    result = p;
                });
        ...
        public ActionResult Approve_Trans(RPTransModel model)
        {
            ...
            try
            {
                ...
                if (model.payment_method_text.Contains("("))
                {
                    model.nosto_vosto_code = model.payment_method_text.Split('(')[1].Replace(")", "");
                }

                api_RPTrans.RPDealSettlement.UpdateRPDealSettlement(model, p =>
                {
                    rwm = p;
                });
                ...
                if (Remove_TransNoFromList(ref StrMsg, ref Next_TransNo, model) == false)
                {
                    Log.WriteLog(Controller, StrMsg);
                    throw new Exception("But Remove TransNoFromList => " + StrMsg);
                }
                ...
                //    if (Check_ReleaseMessage(ref StrMsg, ref IsPayment, model) == false)
                //    {
                //        Log.WriteLog(Controller, StrMsg);
                ...
                //    if (IsPayment)
                //    {
                //        if (!Release_Message(ref StrMsg, model))
                //        {
                //            Log.WriteLog(Controller, StrMsg);
            ...
            Result.Add(new { Success = Status, Message = StrMsg, trans_no = Next_TransNo });
        ...
        public ActionResult UnApprove_Trans(RPTransModel RPTransModel)
        {
            ...
            try
            {
                ...
                api_RPTrans.RPDealSettlement.UpdateRPDealSettlement(RPTransModel, p =>
                {
                    rwm = p;
                });
        ...
        private bool Check_ReleaseMessage(ref string ReturnMsg, ref bool IsPayment, RPTransModel model)
        {
            try
            {
                ...
                ChkPaymentModel.payment_method = model.payment_method;
                ChkPaymentModel.mt_code = model.trans_mt_code;
                ...
                if (IsPayment)
                {
                    ...
                    else if (model.release_printed == "Y")
                    {
                        IsPayment = false;
                    }
                }
        ...
        private bool Release_Message(ref string ReturnMsg, RPTransModel RPTransModel)
        {
            try
            {
                Log.WriteLog(Controller, "Start ReleaseMessage Settlement ==========");
                ...
                RPTransModel.event_type = "Settlement";
                PaymentProcessEntities api_RPReleaseMsg = new PaymentProcessEntities();
                ...
                if (!result.Success)
                {
                    throw new Exception("GenRPReleaseMessage Fail.");
                }
                ...
                //}
                //ReturnMsg = " & Release Message Successfully.";
            }
            catch (Exception ex)
            {
                Log.WriteLog(Controller, $"Release Message Fail : [Trans_No: {RPTransModel.trans_no}]. {ex.Message}");
                //ReturnMsg = "But Release Message Fail ";
            }
            finally
            {
        ...
        private bool Remove_TransNoFromList(ref string ReturnMsg, ref string NextTransNo, RPTransModel RPTransModel)
        {
            try
            {
                int NextIndex;
        ...
        public ActionResult FillCounterPartyPayment(string counterpartyid)
        {
            ...
            api_RPTrans.RPDealSettlement.GetDDLPaymentMethodByCounterPartyID(counterpartyid, p =>
            {
                if (p.Success)
                {
                    res = p.Data.DDLItems;
        ...
        {
            List<DDLItemModel> res = new List<DDLItemModel>();
            api_RPTrans.RPDealSettlement.GetDDLTransMtCode(payment_method, trans_deal_type, cur, repo_deal_type, p =>
             {
                 if (p.Success)
                 {
                     res = p.Data.DDLItems;
