...
namespace GM.Application.Web.Controllers
{
    ...
    public class DWHController : BaseController
    {
        ...
        [HttpPost]
        public JsonResult Search(DWHViewModel modelView)
        {
            try
            {
                DWHModel model = new DWHModel();
                model.file_type = modelView.file_type;
                model.recorded_by = User.UserId;
                ...
                ResultWithModel<DataSet> result = GetDWHList(model);
        ...
        [HttpPost]
        public ActionResult ExportExcel(DWHViewModel modelView)
        {
            try
            {
                DWHModel model = new DWHModel();
                ...
                if (result.Success)
                {
                    if (result.Data != null)
                    {
                        ...
                        Response.Headers.Add("Content-disposition", "attachment; filename=DWH_" + model.file_type + ".xls");
        ...
        public ActionResult FillFileType()
        {
            List<DDLItemModel> ddlListModels = new List<DDLItemModel>();
            StaticEntities api_static = new StaticEntities();
            ResultWithModel<DataSet> result = new ResultWithModel<DataSet>();
            DWHModel model = new DWHModel()
            {
            ...
            api_static.DWH.GetDDLFileType(model, p => {
                result = p;
            });
            ...
            if (result.Success)
            {
                if (result.Data != null)
                {
                    foreach (DataRow row in result.Data.Tables[0].Rows)
                    {
                        DDLItemModel ddlItemModel = new DDLItemModel();
                        ddlItemModel.Text = row["file_type"].ToString();
                        ddlItemModel.Value = ddlItemModel.Text;
                        ddlListModels.Add(ddlItemModel);
                    }
                }
            }

            return Json(ddlListModels, JsonRequestBehavior.AllowGet);
        }
