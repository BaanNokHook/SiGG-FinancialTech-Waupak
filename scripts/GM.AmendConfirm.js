            ...
            swal({
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        ...
                        $.ajax({
                            url: "/RPConfirmation/ReleaseMessageConfirmChange",
                            type: "POST",
                            dataType: "JSON",
                            data: {
                                trans_no: trans_no,
...
function GetMessageConfirm(trans_no, message_tpye) {
    ...
    var data = {
        trans_no: trans_no,
        event_type: 'CHANGE',
        policy_date: $("#FormSearch_policy_date").val(),
        message_type: message_tpye
    };

    $.ajax({
        type: "GET",
        url: "/RPConfirmation/GetMessageConfirmChange",
...
$(document).ready(function () {
    ...
    $("#ddl_policy_rate").click(function () {
        var txt_search = $('#txt_policy_rate');
        var data = { datastr: null };
        GM.Utility.DDLAutoComplete(txt_search, data, null);
        txt_search.val("");
    });

    $('#txt_policy_rate').keyup(function () {
    ...
    GM.MessageConfirm.Table = $('#x-table-data').DataTable({
        ...
        serverSide: true,
        ajax: {
            "url": "/RPConfirmation/SearchConfirm",
            "type": "POST",
            "statusCode": {
                401: function () {
