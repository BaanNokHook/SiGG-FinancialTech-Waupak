$(document).ready(function () {
    ...
    GM.RPDealApprove.Form = {};
    GM.RPDealApprove.Form.Search = function () {
        $('#search-form :input').each(function () {
            var input = $(this);
            var key = input[0].name.split('.')[1];
            switch (key) {
                case "repo_deal_type_code": GM.RPDealApprove.Table.columns(3).search($(this).val()); break;
                case "trans_deal_type": GM.RPDealApprove.Table.columns(4).search($(this).val()); break;
                case "trans_type": GM.RPDealApprove.Table.columns(5).search($(this).val()); break;
                case "port": GM.RPDealApprove.Table.columns(6).search($(this).val()); break;
                case "purpose": GM.RPDealApprove.Table.columns(7).search($(this).val()); break;
                case "counter_party_code": GM.RPDealApprove.Table.columns(8).search($(this).val()); break;
                case "maturity_date": GM.RPDealApprove.Table.columns(12).search($(this).val()); break;
                case "cur": GM.RPDealApprove.Table.columns(16).search($(this).val()); break;
                case "from_trans_no": GM.RPDealApprove.Table.columns(20).search($(this).val()); break;
                case "to_trans_no": GM.RPDealApprove.Table.columns(21).search($(this).val()); break;
                case "from_trade_date": GM.RPDealApprove.Table.columns(22).search($(this).val()); break;
                case "to_trade_date": GM.RPDealApprove.Table.columns(23).search($(this).val()); break;
                case "from_settlement_date": GM.RPDealApprove.Table.columns(24).search($(this).val()); break;
                case "to_settlement_date": GM.RPDealApprove.Table.columns(25).search($(this).val()); break;
                case "from_maturity_date": GM.RPDealApprove.Table.columns(26).search($(this).val()); break;
                case "to_maturity_date": GM.RPDealApprove.Table.columns(27).search($(this).val()); break;
            }
        });

        GM.RPDealApprove.Table.draw();
    };
    GM.RPDealApprove.Form.Initial = function () {
        $("#action-form")[0].reset();
    };
    GM.RPDealApprove.Form.DataBinding = function (p) {
