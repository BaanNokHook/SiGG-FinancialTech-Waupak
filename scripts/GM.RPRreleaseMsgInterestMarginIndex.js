...
function ddlChangePayment(id, value) {
    ...
    var data = { paymentmethod: $('#ddl_payment_' + id).val(), transdealtype: myData[id].margin_type.toUpperCase().indexOf('PAY') === 0 ? 'PAY' : myData[id].margin_type.toUpperCase().indexOf('RECEIVE') === 0 ? 'REV' : '', cur: $('#FormSearch_cur').val() === 'THB' ? $('#FormSearch_cur').val() : 'FCY' };
...
function ReleaseMessage(listData) {
    ...
    $.ajax({
        url: "/RPCallMargin/ReleaseMessageInterestMargin",
        type: "GET",
        dataType: "JSON",
        ...
        success: function (res) {
            ...
            GM.RPReleaseInterestMargin.Search();
...
function ReleaseMessageCyberPay(listData) {
    ...
    $('.spinner').css('display', 'block'); // Open Loading
    $.ajax({
        url: "/RPCallMargin/ReleaseMessageInterestMarginCyberPay",
        type: "GET",
        dataType: "JSON",
        data: { data: JSON.stringify(listData) },
        success: function (res) {
...
function GetMessage(data) {
    ...
    $('.spinner').css('display', 'block'); // Open Loading
    $.ajax({
        url: "/RPReleaseMessage/GetReleaseInterestMargin",
        type: "GET",
        dataType: "JSON",
...
$(document).ready(function () {
    ...
    $('#FormSearch_payment_date_from').data("DateTimePicker").date(callMarginDate);
    $('#FormSearch_payment_date_to').data("DateTimePicker").date(callMarginDate);
    ...
    GM.RPReleaseInterestMargin = {};

    GM.RPReleaseInterestMargin.Table = $('#x-table-data').DataTable({
        ...
        serverSide: true,
        ajax: {
            "url": "/RPReleaseMessage/SearchInterestMargin",
            "type": "POST",
            "statusCode": {
                401: function () {
                    window.location.href = "/RPReleaseMessage";
        ...
        columnDefs:
            [
                ...
                {
                    targets: 3, data: "int_rec_pay_date",
                    render: function (data, type, row, meta) {
                        var html = '';
                        if (data != null) {
                            html = moment(data).format('DD/MM/YYYY');
                        }
                        return '<label id=int_rec_pay_date_' + meta.row + ' style="font-weight:normal" value="' + html + '" >' + html + '</label>';
                    }
                ...
                {
                    targets: 6, data: "int_cash_margin",
                    className: 'dt-body-right',
                    orderable: false,
                    render: function (data, type, row, meta) {
                        var html = '';
                ...
                },
                {
                    ...
                    render: function (data, type, row, meta) {
                        var html = '';
                        html += '<button title="Release Message" class="btn btn-default btn-round" key="' + row.counter_party_id + '" call_date="' + moment(row.int_rec_pay_date).format('DD/MM/YYYY') + '" asof_date="' + moment(row.asof_date).format('DD/MM/YYYY') + '" form-mode="genMessage" id="genMessage_' + meta.row + '" data-action="genMessage"  onclick="GM.RPReleaseInterestMargin.Action(this)" ><i class="feather-icon icon-file-plus"></i></button>';
                        html += '<button title="View Message" class="btn btn-default btn-round" key="' + row.counter_party_id + '" call_date="' + moment(row.int_rec_pay_date).format('DD/MM/YYYY') + '" asof_date="' + moment(row.asof_date).format('DD/MM/YYYY') + '" form-mode="viewMessage" id="viewMessage_' + meta.row + '" data-action="viewMessage"  onclick="GM.RPReleaseInterestMargin.Action(this)" ><i class="feather-icon icon-message-square"></i></button>';
                        return html;
                    }
                },
                { targets: 10, data: "from_as_of_date", visible: false },
                { targets: 11, data: "to_as_of_date", visible: false },
                { targets: 12, data: "from_call_date", visible: false },
                { targets: 13, data: "to_call_date", visible: false },
                { targets: 14, data: "counter_party_id", "visible": false },
                { targets: 15, data: "cur", "visible": false },
                { targets: 16, data: "margin_type", "visible": false },
                { targets: 17, data: "brp_contract_no", "visible": false },
                { targets: 18, data: "payment_date_from", visible: false },
                { targets: 19, data: "payment_date_to", visible: false },
                { targets: 20, data: "int_rec_pay_date", visible: false }
            ],
        fixedColumns: {
            leftColumns: 1,
            rightColumns: 12
        ...
        fnPreDrawCallback: function () {
            //console.log("18 : " + $('#FormSearch_payment_date_from').val());
            //console.log("19 : " + $('#FormSearch_payment_date_to').val());
            $('#x-table-data').DataTable().columns(18).search($('#FormSearch_payment_date_from').val());
            $('#x-table-data').DataTable().columns(19).search($('#FormSearch_payment_date_to').val());
        },
    ...
    GM.RPReleaseInterestMargin.Search = function () {
        try {
            ...
            var FormSearch_payment_date_from = $("#FormSearch_payment_date_from").val();
            var FormSearch_payment_date_to = $("#FormSearch_payment_date_to").val();
            ...
            if (FormSearch_from_as_of_date !== '') {
                GM.RPReleaseInterestMargin.Table.columns(10).search(FormSearch_from_as_of_date);
            }

            if (FormSearch_to_as_of_date !== '') {
                GM.RPReleaseInterestMargin.Table.columns(11).search(FormSearch_to_as_of_date);
            }

            if (FormSearch_from_call_date !== '') {
                GM.RPReleaseInterestMargin.Table.columns(12).search(FormSearch_from_call_date);
            }

            if (FormSearch_to_call_date !== '') {
                GM.RPReleaseInterestMargin.Table.columns(13).search(FormSearch_to_call_date);
            }

            if (FormSearch_payment_date_from !== '') {
                GM.RPReleaseInterestMargin.Table.columns(18).search(FormSearch_payment_date_from);
            }

            if (FormSearch_payment_date_to !== '') {
                GM.RPReleaseInterestMargin.Table.columns(19).search(FormSearch_payment_date_to);
            }

            GM.RPReleaseInterestMargin.Table.columns(14).search(FormSearch_counter_party_id);
            GM.RPReleaseInterestMargin.Table.columns(15).search(FormSearch_cur);

            GM.RPReleaseInterestMargin.Table.draw();
        } catch (err) {
            console.log(err.message);
        }
    };

    GM.RPReleaseInterestMargin.Action = function (btn) {
        ...
        if (mode) {
            ...
            switch (mode) {
                case "viewMessage":
                    ...
                    console.log("rowData[id].payment_method : " + rowData[idViewMessage].payment_method);
                    console.log("rowData[id].mt_code : " + rowData[idViewMessage].mt_code);
                    ...
                    if (rowData[idViewMessage].payment_method === null || rowData[idViewMessage].mt_code === null) {
                        ...
                        var data =
                        {
                            ...
                            call_date: moment(rowData[idViewMessage].int_rec_pay_date).format('YYYY-MM-DD'),
                ...
                case "genMessage":
                    ...
                    console.log("rowData[id].payment_method : " + rowData[idGenMessage].payment_method);
                    console.log("rowData[id].mt_code : " + rowData[idGenMessage].mt_code);
                            ...
                            function (isConfirm) {
                                if (isConfirm) {
                                    ...
                                    var row =
                                    {
                                        ...
                                        call_date: moment(rowData[idGenMessage].int_rec_pay_date).format('YYYY-MM-DD'),
                                    ...
                                    $.post("/RPCallMargin/CheckSettlementStatus", {
                                        counter_party_id: rowData[idGenMessage].counter_party_id,
                                        call_date: moment(rowData[idGenMessage].int_rec_pay_date).format('DD/MM/YYYY'),
                                        payment_method: rowData[idGenMessage].payment_method,
                                        mt_code: rowData[idGenMessage].mt_code,
                                        cur: $('#FormSearch_cur').val(),
                                        row: rowData[idGenMessage].RowNumber,
                                        ...
                                        if (result.Success) {
                                            if (result.Data.ResponseBody[0].SettlementStatus === localStorage.getItem('SettlementStatus.Complete')) {
                                                ...
                                                if ($('#ddl_channel_' + idGenMessage).val() === "CYBER_PAY") {
    ...
    GM.RPReleaseInterestMargin.Sync = function (btn) {
        $('.spinner').css('display', 'block'); // Open Loading

        $.ajax({
            type: "POST",
            url: "/RPCallMargin/SyncTransactionInt",
            content: "application/json; charset=utf-8",
            dataType: "json",
            data: {
                payment_date_from: $('#FormSearch_payment_date_from').val(),
                payment_date_to: $('#FormSearch_payment_date_to').val(),
                cur: $('#FormSearch_cur').val(),
