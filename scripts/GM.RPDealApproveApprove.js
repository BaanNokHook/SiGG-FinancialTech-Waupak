...
function GetPaymentMethod() {
    ...
    if (counterparty_id != "" && counterparty_id != null) {
        ...
        $.ajax({
            url: "/RPDealSettlement/FillCounterPartyPayment",
            type: "GET",
            dataType: "JSON",
            data: data,
            success: function (res) {
                ...
                var cur = $("#Txt_cur").val();
                let data = { payment_method: payment_method, trans_deal_type: trans_deal_type, cur: cur, repo_deal_type: $('#Txt_repo_deal_type').val() };
...
$(document).ready(function () {
    ...
    if ($('#payment_method').val() === undefined || $('#payment_method').val() === "") {
        GetPaymentMethod();
    } else {
        let data = {
    ...
    GM.RPDealApprove.Interest = {};
    GM.RPDealApprove.Interest.Form = $("#check-interest");

    $("#btn_checkinterest").on('click', function () {
        GM.RPDealApprove.Interest.Form.modal('toggle');
    ...
    $('#txt_trans_mt_code').keyup(function () {
        ...
        var payment_method = $("#payment_method").val();
        var cur = $("#Txt_cur").val();
        var data = { payment_method: payment_method, trans_deal_type: trans_deal_type, cur: cur };
