$(document).ready(function () {
    ...
    GM.RPDealMaturity.Form.Search = function () {
        $('#search-form :input').each(function () {
            var input = $(this);
            var key = input[0].name.split('.')[1];
            switch (key) {
                case "repo_deal_type_code": GM.RPDealMaturity.Table.columns(3).search($(this).val()); break;
                case "trans_deal_type": GM.RPDealMaturity.Table.columns(4).search($(this).val()); break;
                case "trans_type": GM.RPDealMaturity.Table.columns(5).search($(this).val()); break;
                case "port": GM.RPDealMaturity.Table.columns(6).search($(this).val()); break;
                case "purpose": GM.RPDealMaturity.Table.columns(7).search($(this).val()); break;
                case "counter_party_code": GM.RPDealMaturity.Table.columns(8).search($(this).val()); break;
                case "maturity_date": GM.RPDealMaturity.Table.columns(12).search($(this).val()); break;
                case "cur": GM.RPDealMaturity.Table.columns(16).search($(this).val()); break;
                case "from_trans_no": GM.RPDealMaturity.Table.columns(20).search($(this).val()); break;
                case "to_trans_no": GM.RPDealMaturity.Table.columns(21).search($(this).val()); break;
                case "from_trade_date": GM.RPDealMaturity.Table.columns(22).search($(this).val()); break;
                case "to_trade_date": GM.RPDealMaturity.Table.columns(23).search($(this).val()); break;
                case "from_settlement_date": GM.RPDealMaturity.Table.columns(24).search($(this).val()); break;
                case "to_settlement_date": GM.RPDealMaturity.Table.columns(25).search($(this).val()); break;
                case "from_maturity_date": GM.RPDealMaturity.Table.columns(26).search($(this).val()); break;
                case "to_maturity_date": GM.RPDealMaturity.Table.columns(27).search($(this).val()); break;
            }
        });

        GM.RPDealMaturity.Table.draw();
    };

    GM.RPDealMaturity.Form.Initial = function () {
        $("#action-form")[0].reset();
    };
