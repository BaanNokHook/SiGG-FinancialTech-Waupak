...
$(document).ready(function () {
    ...
    GM.RPDealSummary.Interest = {};
    GM.RPDealSummary.Interest.Form = $("#check-interest");

    $("#btn_checkinterest").on('click', function () {
        GM.RPDealSummary.Interest.Form.modal('toggle');
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                $.each(res, function (i, resdata) {
                    html += "<tr><td style='text-align:center;'>" + resdata.period
                        + "</td><td style='text-align:center;'>" + resdata.day_period
                        + "</td><td style='text-align:center;'>" + resdata.reference
                        + "</td><td style='text-align:center;'>" + FormatDecimal(resdata.ref_rate)
                        + "</td><td style='text-align:center;'>" + FormatDecimal(resdata.costoffund_spread)
                        + "</td><td style='text-align:center;'>" + FormatDecimal(resdata.costoffund_total_rate)
                        + "</td><td style='text-align:center;'>" + FormatDecimal2(resdata.interest_amount) + "</td></tr>";
                    totalamount += parseFloat(resdata.interest_amount);
                });

                $("#lbl_totalinterest_check").text(FormatDecimal2(totalamount.toFixed(2)));
                body.append(html);

                var repurchase_price = parseFloat($("#purchase_price").val().replace(/,/g, ''));
                repurchase_price += parseFloat(totalamount);

                $("#lbl_repurchase_check").text(FormatDecimal2(repurchase_price));
