...
var treefilter = function(el, options) {
	...
	var defaults = {
		offsetLeft : 0, // left offset for each level
	...
	// Public Variables
	var plugin = this;
    plugin.status = []; // save folder status for "var memory"
    ...
    plugin.Search = function (val) {
        ...
        else
        {
            ...
            plugin.el.find("li:containsIN('" + val + "')").addClass("tf-search-result");
	...
	var memory = function(action) {
		if (action == "in") {
			...
			plugin.el.find("li").each(function() {
                plugin.status.push($(this).hasClass("tf-open"));
			});
		} else if (action == "out") {
